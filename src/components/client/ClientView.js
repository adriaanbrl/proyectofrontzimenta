...
const ClientView = () => {
  ...
  const fetchEvents = async (buildingId, year, month) => {
    ...
    try {
      ...
      if (response.data) {
        const eventsWithDates = response.data.map(event => ({ ...event, fecha: new Date(event.date) }));
  ...
  const { paidAmount: projectPaidAmount, deadlines, projectPhases } = projectData;
  const estimatedPrice = budgetAmount !== null ? budgetAmount : projectData.estimatedPrice || 0;
  const paidAmount = invoiceAmount !== null ? invoiceAmount : projectPaidAmount || 0; // Usar invoiceAmount si está disponible
  ...
  return (
      <Container className="mt-4">
        <Row>
          <Col md={12}>
            <h2 className="text-start mb-4">HOLA, {clientNameFromToken || 'Cliente'}:</h2>
          </Col>
        </Row>
        <Row>
          <Col md={6}>
