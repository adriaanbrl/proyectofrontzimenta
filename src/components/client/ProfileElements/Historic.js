...
const Historic = () => {
  ...
  const resolvedIncidents = incidents
    .filter(
      (incident) =>
        incident.status && incident.status.toLowerCase() === "resuelta"
    )
  ...
  const pendingIncidents = incidents
    .filter(
      (incident) =>
        incident.status && incident.status.toLowerCase() === "pendiente"
    )
    .sort((a, b) => new Date(b.creationDate) - new Date(a.creationDate));
  ...
  const otherIncidents = incidents
    .filter(
      (incident) =>
        incident.status &&
        incident.status.toLowerCase() !== "resuelta" &&
        incident.status.toLowerCase() !== "pendiente"
    )
    .sort((a, b) => new Date(b.creationDate) - new Date(a.creationDate));
  ...
  const handleEditClick = (incident) => {
    ...
    if (
      incident.status.toLowerCase() === "resuelta" ||
      incident.status.toLowerCase() === "cerrada"
    ) {
      alert(
        'No se pueden editar incidentes con estado "Resuelta" o "Cerrada".'
      );
      return;
    }
    ...
    setEditFormData({
      ...
      resolutionDate: incident.resolutionDate
        ? formatDateForInput(incident.resolutionDate)
        : "",
    });
  ...
  const handleDeleteClick = async (incidentId, incidentStatus) => {
    ...
    if (
      incidentStatus.toLowerCase() === "resuelta" ||
      incidentStatus.toLowerCase() === "cerrada"
    ) {
      alert(
        'No se pueden eliminar incidentes con estado "Resuelta" o "Cerrada".'
      );
      return;
    }
    if (
      ...
      <Row className="justify-content-center">
        <Col md={8}>
          <div className="d-flex justify-content-center align-items-center mb-3">
            ...
            <h2 className="historic-title text-center mb-0">
          ...
          {buildingId && incidents.length > 0 ? (
              ...
              {pendingIncidents.length > 0 && (
                <div className="mb-4">
                  ...
                  {pendingIncidents.map((incident) => (
                    <Card
                      key={incident.id}
                      className="incident-card mb-3 shadow-sm"
                    >
                      <Card.Body className="p-3">
                        <Card.Title className="incident-card-title mb-2">
                          {incident.title}
                        </Card.Title>
                        <ListGroup variant="flush">
                          ...
                          <ListGroup.Item>
                            ...
                            <span className="text-muted">
                              {formatDate(incident.creationDate)}
                            </span>
                          </ListGroup.Item>
                        ...
                        <div className="d-flex justify-content-end mt-3">
                          ...
                          <Button
                            ...
                            onClick={() =>
                              handleDeleteClick(incident.id, incident.status)
                            }
            ...
            !error && (
              <p className="text-muted text-center">
                No hay incidentes registrados para este edificio.
              </p>
            )
          )}
        </Col>
