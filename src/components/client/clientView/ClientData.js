...
const ClientData = () => {
    ...
    const [budgetAmount, setBudgetAmount] = useState(0); // Ensure initial state is number
    ...
    const fetchBudget = useCallback(async (buildingId) => {
        ...
        try {
            ...
            // Ensure the amount is a number, default to 0 if not
            setBudgetAmount(Number(response.data?.amount) || 0);
        } catch (err) {
            setBudgetError(err);
            setBudgetAmount(0);
        } finally {
    ...
    const estimatedPrice = Number(budgetAmount || projectData?.estimatedPrice) || 0;
    const paidAmount = Number(invoiceAmount || projectData?.paidAmount) || 0;
    ...
    return {
        ...
        eventMonth,
        setEventMonth,
        budgetAmount: estimatedPrice, // Return the processed estimatedPrice
