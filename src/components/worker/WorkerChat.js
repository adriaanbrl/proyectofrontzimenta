    ...
    if (authToken) {
      try {
        ...
        if (response.ok) {
          ...
          console.error(
            "Error al cargar el historial del chat (Worker):",
            response.status
          );
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (chatContainerRef.current) {
      ...
      if (
        isScrolledToBottom ||
        (messages.length > 0 &&
          messages[messages.length - 1].senderId === workerId)
      ) {
                ...
                >
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor:
                        msg.senderType === "worker" ? "#f5922c" : "#ffffff",
                      color:
                        msg.senderType === "worker" ? "#ffffff" : "#212529",
