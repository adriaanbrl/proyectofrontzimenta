...
const BuildingIncidentsSection = ({ buildingData }) => {
    ...
    const handleIncidentSaved = async (updatedIncidentData) => {
        ...
        try {
            ...
            if (axios.isAxiosError(err) && err.response) {
                setIncidentActionError(`Error al actualizar incidente: ${err.response.data.message || err.message}`);
            } else {
                setIncidentActionError(`Error de red: ${err.message || 'Por favor, revise su conexión.'}`);
            }
        } finally {
            setLoadingIncidentAction(false);
        }
                                ...
                                <ListGroup>
                                    {resolvedAndClosedIncidents.map((incident) => (
                                        ...
                                        >
                                            ...
                                            <div className="mt-3 mt-md-0 d-flex flex-column flex-md-row gap-2"> {/* Added flex layout for buttons */}
                                                <Form.Select
                                                    size="sm"
                                                    value={incident.status}
                                                    onChange={(e) => handleStatusChange(incident.id, e.target.value)}
                                                ...
                                                >
                                                    {loadingIncidentAction ? <Spinner animation="border" size="sm" /> : '✏️ Editar'}
                                                </Button>
                                                {updatingStatus && <Spinner animation="border" size="sm" className="ms-2" />}
                                            </div>
                                        </ListGroup.Item>
